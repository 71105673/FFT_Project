...
set fft_clk_period "[expr {$per500*$CLK_MGN}]" 
set fft_clk_period_h "[expr {$fft_clk_period/2.0}]"

### I/O DELAY per clock speed
set fft_clk_delay         [expr "$per500 * $CLK_MGN * $io_dly"] 
...
create_clock -name fft_clk       -period $fft_clk_period   -waveform "0 $fft_clk_period_h" [get_ports clk]
...
#set_output_delay   -0.7    -clock rrc_clk  [get_ports clk]
#set_output_delay   -700    -clock rrc_clk  [get_ports clk]
#set_output_delay   700    -clock rrc_clk  [get_ports clk]

#(RXDIN  )  -- Setup/Hold
#set_input_delay     0.5    -clock rrc_clk  [get_ports clk]
set_input_delay     500    -clock fft_clk  [get_ports clk]
