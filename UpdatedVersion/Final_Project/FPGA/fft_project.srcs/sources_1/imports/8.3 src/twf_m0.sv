module twf512 (
    ...
    logic [4:0] cnt;
    logic signed [8:0] twf_re [0:511];  // ?????? ??? ROM ??????
        ...
        if (~rst) begin
            ...
            case (cnt)
                0:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[   0 + j];
                        twf_r_sub[j] <= twf_re[  64 + j];
                        twf_i_add[j] <= twf_im[   0 + j];
                        twf_i_sub[j] <= twf_im[  64 + j];
                    end
                1:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[  16 + j];
                        twf_r_sub[j] <= twf_re[  80 + j];
                        twf_i_add[j] <= twf_im[  16 + j];
                        twf_i_sub[j] <= twf_im[  80 + j];
                    end
                2:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[  32 + j];
                        twf_r_sub[j] <= twf_re[  96 + j];
                        twf_i_add[j] <= twf_im[  32 + j];
                        twf_i_sub[j] <= twf_im[  96 + j];
                    end
                3:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[  48 + j];
                        twf_r_sub[j] <= twf_re[ 112 + j];
                        twf_i_add[j] <= twf_im[  48 + j];
                        twf_i_sub[j] <= twf_im[ 112 + j];
                    end
                4:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[ 128 + j];
                        twf_r_sub[j] <= twf_re[ 192 + j];
                        twf_i_add[j] <= twf_im[ 128 + j];
                        twf_i_sub[j] <= twf_im[ 192 + j];
                    end
                5:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[ 144 + j];
                        twf_r_sub[j] <= twf_re[ 208 + j];
                        twf_i_add[j] <= twf_im[ 144 + j];
                        twf_i_sub[j] <= twf_im[ 208 + j];
                    end
                6:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[ 160 + j];
                        twf_r_sub[j] <= twf_re[ 224 + j];
                        twf_i_add[j] <= twf_im[ 160 + j];
                        twf_i_sub[j] <= twf_im[ 224 + j];
                    end
                7:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[ 176 + j];
                        twf_r_sub[j] <= twf_re[ 240 + j];
                        twf_i_add[j] <= twf_im[ 176 + j];
                        twf_i_sub[j] <= twf_im[ 240 + j];
                    end
                8:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[256 + j];
                        twf_r_sub[j] <= twf_re[320 + j];
                        twf_i_add[j] <= twf_im[256 + j];
                        twf_i_sub[j] <= twf_im[320 + j];
                    end
                9:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[272 + j];
                        twf_r_sub[j] <= twf_re[336 + j];
                        twf_i_add[j] <= twf_im[272 + j];
                        twf_i_sub[j] <= twf_im[336 + j];
                    end
                10:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[288 + j];
                        twf_r_sub[j] <= twf_re[352 + j];
                        twf_i_add[j] <= twf_im[288 + j];
                        twf_i_sub[j] <= twf_im[352 + j];
                    end
                11:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[304 + j];
                        twf_r_sub[j] <= twf_re[368 + j];
                        twf_i_add[j] <= twf_im[304 + j];
                        twf_i_sub[j] <= twf_im[368 + j];
                    end
                12:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[384 + j];
                        twf_r_sub[j] <= twf_re[448 + j];
                        twf_i_add[j] <= twf_im[384 + j];
                        twf_i_sub[j] <= twf_im[448 + j];
                    end
                13:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[400 + j];
                        twf_r_sub[j] <= twf_re[464 + j];
                        twf_i_add[j] <= twf_im[400 + j];
                        twf_i_sub[j] <= twf_im[464 + j];
                    end
                14:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[416 + j];
                        twf_r_sub[j] <= twf_re[480 + j];
                        twf_i_add[j] <= twf_im[416 + j];
                        twf_i_sub[j] <= twf_im[480 + j];
                    end
                15:  for (j = 0; j < 16; j++) begin
                        twf_r_add[j] <= twf_re[432 + j];
                        twf_r_sub[j] <= twf_re[496 + j];
                        twf_i_add[j] <= twf_im[432 + j];
                        twf_i_sub[j] <= twf_im[496 + j];
                    end
