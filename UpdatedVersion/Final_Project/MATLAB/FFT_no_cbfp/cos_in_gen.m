...
function [data_float, data_fixed] = cos_in_gen(fft_mode, num)
 ...
 for i=1:N
  ...
  fp = fopen('cosine_table.txt', 'w');
fprintf(fp, 'Index\tFloat_Real\tFloat_Imag\tFixed_Real\tFixed_Imag\n');
for i = 1:N
    fprintf(fp, '%d\t%.10f\t%.10f\t%.10f\t%.10f\n', ...
        i, real(data_float(i)), imag(data_float(i)), ...
        real(data_fixed(i)/64), imag(data_fixed(i)/64));
end
fclose(fp);
