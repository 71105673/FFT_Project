% Added on 2024/07/02 by jihan 
function [fft_out, module2_out] = fft_fixed_stu(fft_mode, fft_in)

 shift = 0;

	din = fft_in; % <2.7> => <3.6>

 fac8_0 = [1, 1, 1, -j];
 fac8_2 = [1, 1, 1, -j, 1, 0.7071-0.7071j, 1, -0.7071-0.7071j]; % floating
 fac8_1 = round(fac8_2 * 256); % fixed <2.8>

 %-----------------------------------------------------------------------------
 % Module 0
 %-----------------------------------------------------------------------------
 % step0_0
 % m1: din - 9bit; bfly00 - 10bit
 bfly00_out0 = din(1:256) + din(257:512); %INPUT <3.6> -> OUTPUT <4.6>
 bfly00_out1 = din(1:256) - din(257:512);

 save_table_single(bfly00_out1, 1,'bfly00_out1_fixed.txt' );

 bfly00_tmp = [bfly00_out0, bfly00_out1];

 for nn=1:512
	bfly00(nn) = bfly00_tmp(nn)*fac8_0(ceil(nn/128)); %INPUT <4.6> -> OUTPUT <4.6>
 end

 save_table_single(bfly00, 1,'bfly00_fixed.txt' );


 % step0_1
 % m1: bfly00 - 10bit; bfly01_tmp - 11bit; bfly01 - 12bit
 for kk=1:2
  for nn=1:128
	bfly01_tmp((kk-1)*256+nn) = bfly00((kk-1)*256+nn) + bfly00((kk-1)*256+128+nn); %INPUT <4.6> -> OUTPUT <5.6>
	bfly01_tmp((kk-1)*256+128+nn) = bfly00((kk-1)*256+nn) - bfly00((kk-1)*256+128+nn);
  end
 end

 save_table_single(bfly01_tmp, 1,'bfly01_tmp_fixed.txt' );

 fp_1=fopen('bfly01.txt','w');
 for nn=1:512
	temp_bfly01(nn) = bfly01_tmp(nn)*fac8_1(ceil(nn/64));  %INPUT <5.6> X <2.8> -> OUTPUT <7.14>
	bfly01(nn) = round(temp_bfly01(nn)/256); % <7.6>
	fprintf(fp_1, 'bfly01_tmp(%d)=%d+j%d, temp_bfly01(%d)=%d+j%d, bfly01(%d)=%d+j%d\n',nn, real(bfly01_tmp(nn)), imag(bfly01_tmp(nn)),nn,real(temp_bfly01(nn)),imag(temp_bfly01(nn)),nn,real(bfly01(nn)),imag(bfly01(nn)));
 end
 fclose(fp_1);

save_table_single(bfly01, 1,'bfly01_fixed.txt' );


 % step0_2
 % m1: bfly01 - 12bit; bfly02_tmp - 13bit; pre_bfly02 - 14bit; bfly02 - 11bit
 for kk=1:4
  for nn=1:64
	bfly02_tmp((kk-1)*128+nn) = bfly01((kk-1)*128+nn) + bfly01((kk-1)*128+64+nn); %INPUT <7.6> -> OUTPUT <8.6>
	bfly02_tmp((kk-1)*128+64+nn) = bfly01((kk-1)*128+nn) - bfly01((kk-1)*128+64+nn);
  end
 end

  save_table_single(bfly02_tmp, 1,'bfly02_tmp_fixed.txt' );


 for ii=1:512
 	%bfly02_tmp = sat(bfly02_tmp, 13); % Saturatin (13 bit)
 	bfly02_tmp(ii) = sat(bfly02_tmp(ii), 13); % Saturatin (13 bit):  % <8.6> -> <7.6>
 end

 % Data rearrangement  
 % bit-reversal 순서를 기반으로 만든 twiddle factor 인덱스 순서
 K3 = [0, 4, 2, 6, 1, 5, 3, 7];

 % Twiddle factor
 for kk=1:8 % FFT 길이가 512고, 이걸 8개의 블록으로 나눴을 때 몇 번째 블록인지를 의미
  for nn=1:64 % 각 블록 내부에서 몇 번째 데이터인지를 의미
      %index 예시) kk=3, nn=17, (kk-1)*64 + nn = 2*64 + 17 = 145, twf_m0(145)
	flo_twf_m0((kk-1)*64+nn) = exp(-j*2*pi*(nn-1)*(K3(kk))/512);
	twf_m0((kk-1)*64+nn) = round(flo_twf_m0((kk-1)*64+nn)*128); % twf_m0 : <2.7>
  end
 end

 
cbfp0_in = fopen("cbfp_in.txt",'w');
 % multiply Twiddle factor
 for nn=1:512
	%bfly02(nn) = bfly02_tmp(nn)*twf_m0(nn); % Org 
	pre_bfly02(nn) = bfly02_tmp(nn)*twf_m0(nn); % (14bit(13+1) * 9bit = 23 bit) , <10.13> // +1은 CBFP 연산을 위한 부호 비트를 더하는 것이라 그냥 암기....
    fprintf(cbfp0_in,'%d\n',real(pre_bfly02(nn)));
 end

 save_table_single(pre_bfly02, 1,'pre_bfly02_fixed.txt' );


 % CBFP(Convergent Block Floating Point) stage0
 % cnt1 =3으로 가정
 cnt1_re = zeros(1,8);
 cnt1_im = zeros(1,8);
 for ii=1:8
  for jj=1:64
	tmp1_re=mag_detect(real(pre_bfly02(64*(ii-1)+jj)),23);
	tmp1_im=mag_detect(imag(pre_bfly02(64*(ii-1)+jj)),23);

    % mag_detect: MSB(왼쪽)부터 각 데이터가 얼마나 시프트될 수 있는지를 계산 
    % (leading 0 또는 1의 개수 기반)

	temp1_re=min_detect(jj,tmp1_re,cnt1_re(ii));
	temp1_im=min_detect(jj,tmp1_im,cnt1_im(ii));

    % min_detect: 64개의 데이터 중 시프트가 가장 덜 되는 값을 찾는다.
    % 이전까지 본 값 중 가장 큰 bit 수를 계속 유지 
    % 각 블록(64개 단위)마다 최대한 안전하게 시프트할 수 있는 폭을 계산해, 
    % 해당 폭만큼 전부 정규화(bit shift)한다.

	cnt1_re(ii)=temp1_re;
	cnt1_im(ii)=temp1_im;
  end
 end

 % 실수/허수 중 더 작은 값을 채택(즉, 더 적게 shift할 수 있는 값)
 % 정규화 시 둘 중 더 작은 쪽을 기준으로 bit shift 해야 overflow 안 생김
 for ii=1:8
  if (cnt1_re(ii)<=cnt1_im(ii))
	cnt1_re(ii)=cnt1_re(ii);
  else
	cnt1_re(ii)=cnt1_im(ii);
  end
 end

 fp_2=fopen('minvalue.txt','w');
 for ii=1:8
  if (cnt1_im(ii)<=cnt1_re(ii))
	cnt1_im(ii)=cnt1_im(ii);
  else
	cnt1_im(ii)=cnt1_re(ii);
  end
 end

 % 각 포인트별로 그 블록의 scaling bit 복사
 for ii=1:8
  for jj=1:64
	index1_re(64*(ii-1)+jj)=cnt1_re(ii);
	index1_im(64*(ii-1)+jj)=cnt1_im(ii);
  end
 end

 % bit shift 정규화
 for ii=1:8
  for jj=1:64
   if (cnt1_re(ii)>12) % bitshift <10.13> -> 11bit 
	re_bfly02(64*(ii-1)+jj)=bitshift(bitshift(real(pre_bfly02(64*(ii-1)+jj)),cnt1_re(ii), 'int32'),-12, 'int32');
   else
	re_bfly02(64*(ii-1)+jj)=bitshift(real(pre_bfly02(64*(ii-1)+jj)),(-12+cnt1_re(ii)), 'int32'); % if cnt1 = 3, <10.13> -> <7.4> 

    % 0개수는 부호비트 1개 + cnt1, 소수부는 계산 편하게 1로 통일
    % 00_0011_1111.1111_1111_1111_1111_1 -> (cnt1 shift): 011_1111.1111_1111_1111_1111_1000 -> (-12 shift): 011_1111.1111 <7.4> 
   end
  end
 end

 for ii=1:8
  for jj=1:64
   if (cnt1_im(ii)>12)
	im_bfly02(64*(ii-1)+jj)=bitshift(bitshift(imag(pre_bfly02(64*(ii-1)+jj)),cnt1_im(ii), 'int32'),-12, 'int32');
   else
	im_bfly02(64*(ii-1)+jj)=bitshift(imag(pre_bfly02(64*(ii-1)+jj)),(-12+cnt1_im(ii)), 'int32'); % 실수와 동일 <7.4>
   end
  end
 end
 % 최종적으로 12bit을 오른쪽으로 밀면 아래 11bit에만 유효한 숫자가 존재하긴 함

 fp_2=fopen('cbfp_0.txt','w');
 for nn=1:512
	bfly02(nn) = re_bfly02(nn) + j*im_bfly02(nn);  
	fprintf(fp_2, 'twf_m0(%d)=%d+j%d, pre_bfly02(%d)=%d+j%d, index1_re(%d)=%d, index1_im(%d)=%d, bfly02(%d)=%d+j%d\n',nn, real(twf_m0(nn)), imag(twf_m0(nn)), nn, real(pre_bfly02(nn)), imag(pre_bfly02(nn)), nn, index1_re(nn), nn, index1_im(nn), nn, real(bfly02(nn)), imag(bfly02(nn)));
 end
 fclose(fp_2);

 save_table_single(bfly02, 1,'bfly02_fixed.txt');


 
 %-----------------------------------------------------------------------------
 % Module 1
 %-----------------------------------------------------------------------------
 % step1_0
 % m1: bfly02 - 11bit; bfly10 - 12bit;

 for kk=1:8
  for nn=1:32
	bfly10_tmp((kk-1)*64+nn) = bfly02((kk-1)*64+nn) + bfly02((kk-1)*64+32+nn); % INPUT <7.4> -> OUTPUT <8.4>
	bfly10_tmp((kk-1)*64+32+nn) = bfly02((kk-1)*64+nn) - bfly02((kk-1)*64+32+nn);
  end
 end

 for ii=1:512
 	bfly10_tmp(ii) = sat(bfly10_tmp(ii), 12); % Saturatin (12 bit) 
 end

  save_table_single(bfly10_tmp, 1,'bfly10_tmp_fixed.txt' );


 for kk=1:8
  for nn=1:64
	bfly10((kk-1)*64+nn) = bfly10_tmp((kk-1)*64+nn)*fac8_0(ceil(nn/16));  % INPUT <8.4> -> OUTPUT <8.4>
  end
 end

  save_table_single(bfly10, 1,'bfly10_fixed.txt' );


 % step1_1
 % m1: bfly10 - 12bit; bfly11_tmp - 13bit; bfly11 - 14bit;
 for kk=1:16
  for nn=1:16
	bfly11_tmp((kk-1)*32+nn) = bfly10((kk-1)*32+nn) + bfly10((kk-1)*32+16+nn);  % INPUT <8.4> -> OUTPUT <9.4>
	bfly11_tmp((kk-1)*32+16+nn) = bfly10((kk-1)*32+nn) - bfly10((kk-1)*32+16+nn);
  end
 end

 save_table_single(bfly11_tmp, 1,'bfly11_tmp_fixed.txt');


 for kk=1:8
  for nn=1:64 % 개큰 이슈 bfly11_tmp -> 13bit -> 12bit해줘야 함
	temp_bfly11((kk-1)*64+nn) = bfly11_tmp((kk-1)*64+nn)*fac8_1(ceil(nn/8)); % INPUT <9.4> x <2.8> -> OUTPUT <11.12>
	bfly11((kk-1)*64+nn) = round(temp_bfly11((kk-1)*64+nn)/256); % INPUT <11.12> -> OUTPUT <11.4>
  end
 end

  save_table_single(bfly11, 1,'bfly11_fixed.txt');



 fp_3=fopen('bfly11.txt','w');
 for nn=1:512
	fprintf(fp_3, 'bfly11_tmp(%d)=%d+j%d, temp_bfly11(%d)=%d+j%d, bfly11(%d)=%d+j%d\n',nn, real(bfly11_tmp(nn)), imag(bfly11_tmp(nn)),nn, real(temp_bfly11(nn)), imag(temp_bfly11(nn)),nn, real(bfly11(nn)), imag(bfly11(nn)));
 end
 fclose(fp_3);


 % step1_2
 % m1: bfly11 - 14bit; bfly12_tmp - 15bit; pre_bfly12 - 16bit; bfly12 - 12bit;
 for kk=1:32
  for nn=1:8
	bfly12_tmp((kk-1)*16+nn) = bfly11((kk-1)*16+nn) + bfly11((kk-1)*16+8+nn); % INPUT <11.4> -> OUTPUT <12.4>
	bfly12_tmp((kk-1)*16+8+nn) = bfly11((kk-1)*16+nn) - bfly11((kk-1)*16+8+nn);
  end
 end

 save_table_single(bfly12_tmp, 1,'bfly12_tmp_fixed.txt');


 % Data rearrangement
 K2 = [0, 4, 2, 6, 1, 5, 3, 7];

 for kk=1:8
  for nn=1:8
	flo_twf_m1((kk-1)*8+nn) = exp(-j*2*pi*(nn-1)*(K2(kk))/64);
	twf_m1((kk-1)*8+nn) = round(flo_twf_m1((kk-1)*8+nn)*128); % twf_m0 : <2.7>
  end
 end

 

 for kk=1:8
  for nn=1:64
	pre_bfly12((kk-1)*64+nn) = bfly12_tmp((kk-1)*64+nn)*twf_m1(nn); % (16bit(15+1) * 9bit = 25 bit) 이때 무조건 25bit가 들어가야 계산이 맞음.
    %그래서 첫번째 twiddle factor를 <1.8>로 생각하고 saturation을 무시해서 25bit로 입력되게 만들어줘야
    %함
  end
 end

 % CBFP(Convergent Block Floating Point) stage1 
 % cnt2 =6으로 가정
 cnt2_re = zeros(1,64);
 cnt2_im = zeros(1,64);
 for ii=1:64
  for jj=1:8
	tmp2_re=mag_detect(real(pre_bfly12(8*(ii-1)+jj)),25);
	tmp2_im=mag_detect(imag(pre_bfly12(8*(ii-1)+jj)),25);
	temp2_re=min_detect(jj,tmp2_re,cnt2_re(ii));
	temp2_im=min_detect(jj,tmp2_im,cnt2_im(ii));
	cnt2_re(ii)=temp2_re;
	cnt2_im(ii)=temp2_im;
  end
 end

 %{ 
 for ii=1:64
	X=sprintf('cnt2_re(%d)=%d\n', ii, cnt2_re(ii));
	disp(X);
 end
 %}

 for ii=1:64
  if (cnt2_re(ii)<=cnt2_im(ii))
	cnt2_re(ii)=cnt2_re(ii);
  else
	cnt2_re(ii)=cnt2_im(ii);
  end
 end
		
 for ii=1:64
  if (cnt2_im(ii)<=cnt2_re(ii))
	cnt2_im(ii)=cnt2_im(ii);
  else
	cnt2_im(ii)=cnt2_re(ii);
  end
 end

 for ii=1:64
  for jj=1:8
	index2_re(8*(ii-1)+jj)=cnt2_re(ii);
	index2_im(8*(ii-1)+jj)=cnt2_im(ii);
  end
 end

 
 for ii=1:64
  for jj=1:8
   if (cnt2_re(ii)>13)
	re_bfly12(8*(ii-1)+jj)=bitshift(bitshift(real(pre_bfly12(8*(ii-1)+jj)),cnt2_re(ii), 'int32'),-13, 'int32');
   else
	re_bfly12(8*(ii-1)+jj)=bitshift(real(pre_bfly12(8*(ii-1)+jj)),(-13+cnt2_re(ii)), 'int32'); % if cnt2 = 6, <15.11> -> <9.4>

    % 0개수는 부호비트 1개 + cnt1, 소수부는 계산 편하게 1로 통일
    % 000_0000_1111_1111.1111_1111_111 -> (cnt2 shift): 0_1111_1111.1111_1111_1110_0000_0 -> (-13 shift): 0_1111_1111.1111 <9.4> 

   end
  end
 end

 for ii=1:64
  for jj=1:8
   if (cnt2_im(ii)>13)
	im_bfly12(8*(ii-1)+jj)=bitshift(bitshift(imag(pre_bfly12(8*(ii-1)+jj)),cnt2_im(ii), 'int32'),-13, 'int32');
   else
	im_bfly12(8*(ii-1)+jj)=bitshift(imag(pre_bfly12(8*(ii-1)+jj)),(-13+cnt2_im(ii)), 'int32');
   end
  end
 end


 fp_4=fopen('cbfp_1.txt','w');
 for nn=1:512
	bfly12(nn) = re_bfly12(nn) + j*im_bfly12(nn);  
	%fprintf(fp_4, 'twf_m1(%d)=%d+j%d, pre_bfly12(%d)=%d+j%d, index2_re(%d)=%d, index2_im(%d)=%d, bfly12(%d)=%d+j%d\n',nn, real(twf_m1(nn)), imag(twf_m1(nn)), nn, real(pre_bfly12(nn)), imag(pre_bfly12(nn)), nn, index2_re(nn), nn, index2_im(nn), nn, real(bfly12(nn)), imag(bfly12(nn)));
	fprintf(fp_4, 'pre_bfly12(%d)=%d+j%d, index2_re(%d)=%d, index2_im(%d)=%d, bfly12(%d)=%d+j%d\n', nn, real(pre_bfly12(nn)), imag(pre_bfly12(nn)), nn, index2_re(nn), nn, index2_im(nn), nn, real(bfly12(nn)), imag(bfly12(nn)));
 end
 fclose(fp_4);

  save_table_single(bfly12, 1,'bfly12_fixed.txt');




 %-----------------------------------------------------------------------------
 % Module 2
 %-----------------------------------------------------------------------------
 % step2_0
 % m1: bfly12 - 12bit; bfly20 - 13bit;
 for kk=1:64
  for nn=1:4
	bfly20_tmp((kk-1)*8+nn) = bfly12((kk-1)*8+nn) + bfly12((kk-1)*8+4+nn); % INPUT <9.4> -> OUTPUT <10.4>
	bfly20_tmp((kk-1)*8+4+nn) = bfly12((kk-1)*8+nn) - bfly12((kk-1)*8+4+nn);
  end
 end

 save_table_single(bfly20_tmp, 1,'bfly20_tmp_fixed.txt' );

 for kk=1:64
  for nn=1:8
	bfly20((kk-1)*8+nn) = bfly20_tmp((kk-1)*8+nn)*fac8_0(ceil(nn/2)); % INPUT <10.4> -> OUTPUT <10.4>
  end
 end

 save_table_single(bfly20, 1,'bfly20_fixed.txt' );


 % step2_1
 % m1: bfly20 - 13bit; bfly21_tmp - 14bit; bfly21 - 15bit;
 for kk=1:128
  for nn=1:2
	bfly21_tmp((kk-1)*4+nn) = bfly20((kk-1)*4+nn) + bfly20((kk-1)*4+2+nn); % INPUT <10.4> -> OUTPUT <11.4>
	bfly21_tmp((kk-1)*4+2+nn) = bfly20((kk-1)*4+nn) - bfly20((kk-1)*4+2+nn);
  end
 end

 for ii=1:512
 	bfly21_tmp(ii) = sat(bfly21_tmp(ii), 14); % Saturatin: 정수부 절삭 (14 bit) % INPUT <11.4> -> OUTPUT <10.4>
 end

 save_table_single(bfly21_tmp, 1,'bfly21_tmp_fixed.txt' );


 for kk=1:64
  for nn=1:8
	temp_bfly21((kk-1)*8+nn) = bfly21_tmp((kk-1)*8+nn)*fac8_1(nn); % INPUT <10.4> x <2.8> -> OUTPUT <12.12> 
	bfly21((kk-1)*8+nn) = round(temp_bfly21((kk-1)*8+nn)/256);  % INPUT <12.12> -> OUTPUT <12.4>
  end
 end

 save_table_single(bfly21, 1,'bfly21_fixed.txt' );


 fp_5=fopen('bfly21.txt','w');
 for nn=1:512
	fprintf(fp_5, 'bfly21_tmp(%d)=%d+j%d, temp_bfly21(%d)=%d+j%d, bfly21(%d)=%d+j%d\n',nn, real(bfly21_tmp(nn)), imag(bfly21_tmp(nn)),nn, real(temp_bfly21(nn)), imag(temp_bfly21(nn)),nn, real(bfly21(nn)), imag(bfly21(nn)));
 end
 fclose(fp_5);

 % step2_2
 % m1: bfly21 - 15bit; bfly22_tmp - 16bit; bfly22 - 13bit;
 for kk=1:256
	bfly22_tmp((kk-1)*2+1) = bfly21((kk-1)*2+1) + bfly21((kk-1)*2+2); % INPUT <12.4> -> OUTPUT <13.4>
	bfly22_tmp((kk-1)*2+2) = bfly21((kk-1)*2+1) - bfly21((kk-1)*2+2);
 end

 for ii=1:512
 	bfly22_tmp(ii) = sat(bfly22_tmp(ii), 16); % Saturatin (16 bit) % INPUT <13.4> -> OUTPUT <12.4>
 end

  save_table_single(bfly22_tmp, 1,'bfly22_tmp_fixed.txt' );


 for kk=1:512
	indexsum_re(kk)=index1_re(kk)+index2_re(kk); % 3 + 6 = 9
	indexsum_im(kk)=index1_im(kk)+index2_im(kk);
 end


if (fft_mode == 0)
    fft_coeff = 6; %ifft <10.3>
else
    fft_coeff = 9; % fft<9.4>
end


 for ii=1:512
  if (indexsum_re(ii)>=23)
	re_bfly22(ii) = 0;
  else
	re_bfly22(ii) = bitshift(real(bfly22_tmp(ii)), (fft_coeff-indexsum_re(ii)), 'int32');  % INPUT <12.4> -> OUTPUT <12.4> % 16bit => 13bit <8.5> => <9.4> (FFT) 
  end
 end

 for ii=1:512
  if (indexsum_im(ii)>=23)
	im_bfly22(ii) = 0;
  else
	im_bfly22(ii) = bitshift(imag(bfly22_tmp(ii)), (fft_coeff-indexsum_im(ii)), 'int32'); % 16bit => 13bit <8.5> (FFT)
  end
 end

 for nn=1:512
	bfly22(nn) = re_bfly22(nn) + j*im_bfly22(nn);
 end

 save_table_single(bfly22, 1,'bfly22_fixed.txt' );

 %bfly22 = bfly22_tmp;

 %-----------------------------------------------------------------------------
 % Index 
 %-----------------------------------------------------------------------------
 fp=fopen('fxd_reorder_index.txt','w');
 for jj=1:512
	%kk = bitget(jj-1,9)*(2^0) + bitget(jj-1,8)*(2^1) + bitget(jj-1,7)*(2^2) + bitget(jj-1,6)*(2^3) + bitget(jj-1,5)*(2^4) + bitget(jj-1,4)*(2^5) + bitget(jj-1,3)*(2^6) + bitget(jj-1,2)*(2^7) + bitget(jj-1,1)*(2^8);
	kk = bitget(jj-1,9)*1 + bitget(jj-1,8)*2 + bitget(jj-1,7)*4 + bitget(jj-1,6)*8 + bitget(jj-1,5)*16 + bitget(jj-1,4)*32 + bitget(jj-1,3)*64 + bitget(jj-1,2)*128 + bitget(jj-1,1)*256;
	dout(kk+1) = bfly22(jj); % With reorder
	%fprintf(fp, 'jj=%d, kk=%d, dout(%d)=%d+j%d, indexsum=%d\n',jj, kk,(kk+1),real(dout(kk+1)),imag(dout(kk+1)),indexsum_re(kk+1));
 end
 fclose(fp);

	fft_out = dout;
	module2_out = bfly22;
 

end
