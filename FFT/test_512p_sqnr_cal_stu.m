...
% Added on 2025/07/02 by jihan 
 ...
 % mat_float_fft = fft(cos_float); % Matlab fft (Cosine, Floating-point)
  ...
  [fft_out_fixed, module2_out_fixed] = fft_fixed_3(1, ran_fixed); % Fixed-point fft (Random, fft)
 % [fft_out_fixed, module2_out_fixed] = fft_fixed_3(1, cos_fixed); % Fixed-point fft (Cosine, fft)
 fft_out_fixed = fft_out_fixed/16; % Modified on 2025/07/02 by jihan
  ...
  for ii=1:N
	...
	fprintf(fp_1,'sig_pow(ii = %d)=%f, noise_pow(ii)=%f, fft_out_fixed=%f\n', ii, sig_pow(ii), power(noise_re(ii),2), power(noise_im(ii),2)); 
  end
  fclose(fp_1);
  ...
  % snr_val = 10*log10(double(tot_sig_pow)/double(tot_noise_pow));
